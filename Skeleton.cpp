...
BOOL CSkeleton::CalcSkinMatrice(IMatrice43*mats, BYTE *affects = NULL, DWORD nAffects = 0)
{
	assert(_si.size()==mats->GetCount());
    ...
    {
        for (int i = 0;i < nAffects;i++)
        {
            int idx = (int)affects[i];
            bones[idx] = _si[idx].matOff*bones[idx];
        }
    }
